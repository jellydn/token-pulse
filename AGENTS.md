...
- Run from the repository root. Static assets use relative paths to `public/app.css` and `node_modules/htmx.org/dist/htmx.min.js`; production runs `bun dist/index.js` from the install root with those present, so `dist/` alone is not a complete deployment. CSS is generated and ignored by Git.
...
- `TOKEN_PULSE_SOURCE` selects `demo` (default), `http`, or `cli`. Starting the app collects and writes data. `bun run seed` always writes demo data to the configured database, regardless of source mode.
